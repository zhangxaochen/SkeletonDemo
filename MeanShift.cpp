...
vector<double> MeanShift::shift_point(const vector<double> &point, const vector<vector<double> > &points, const vector<double> &weights, double kernel_bandwidth) {
    ...
    bool add_weight=true;
    if (weights.size()!=points.size()/point.size())
    {
        add_weight=false;
    }
    ...
    do {
        ...
        for(int i=0; i<shifted_points.size(); i++){
            if (!stop_moving[i]) {
                ...
                vector<double>point_new = shift_point(shifted_points[i], points, weights, kernel_bandwidth);
